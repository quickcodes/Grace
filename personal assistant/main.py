...
bool_thank, bool_im, bool_by, bool_wow, bool_no = True, True, True, True, True
bool1, bool2, bool3, bool4, bool5, bool6, bool7 = True, True, True, True, True, True, True
...
def respond(query):
    ...
    global bool_math, bool_shot, bool_read_write, bool_featuers, bool_im, bool_thank, bool_by, bool_wow, bool_no
    global bool1, bool2, bool3, bool4, bool5, bool6, bool7
    try:
        # HELLO
        ...
        if bool_songs and exist(['sing a song', 'i want to dance with you']):
            ...
            # SAD
                ...
                if bool:
                    ...
                    youtube_auto()
            # CALM
            elif exist(['something calm', 'calm songs', 'calm song', 'calm',
                        'something kaam', 'kaam songs', 'kaam song', 'kaam',
                        'something kamal', 'kamal songs', 'kamal song', 'kamal']):
                txt = ['okay', 'sure', 'yes', 'yup', 'Yay', 'wow', 'exciting', 'hurrah']
                word = txt[random.randint(0, len(txt) - 1)]
                speak(f"{word} {user.name} I then now its my responsitility to maintaine your mood")
                playsongs.PlaySongs.calm()
                time.sleep(6)
                if bool:
                    keyboard.press("f")
                else:
        ...
        # TELL ME ------------------------------------------------------------------------------------------------------
        # MEANING
        if bool2 and exist(['dsa', 'data structure']):
            bool2 = False
            webbrowser.open("https://github.com/quickcodes/DSA")
            speak("You can Learn and recape Data Structure Frome Here")
        ...
        if bool_syn and exist(['synonym']):
            ...
            bool_syn = False
        # DAY, MONTH, YEAR
        elif bool_syn and exist(['current', 'right now', 'tell me', 'what is']) and exist(
                ['day', 'date', 'year', 'week', 'month']):
            daytime.DayTime.tell_dmy()
            bool_syn = False
        # TIME
        elif bool_syn and exist(['current', 'right now', 'tell me', 'what is']) and ("time" in query):
            bool_syn = False
            daytime.DayTime.tell_time()
        # JOKE
        elif bool_syn and ("joke" in query) and ("tell"):
            bool_syn = False
            print("inside joke")
            speak(Reply.okay())
            tell_joke()
            ...
            # speak(Reply.laugh())
        # NEWS
        elif bool_syn and ("news" in query) and ("tell"):
            bool_syn = False
            speak(Reply.okay())
            tell_news()
        # PRICE OF
        elif bool_syn and exist(['price of']) and exist(['tell', 'what', 'show']):
            bool_syn = False
            speak(Reply.okay())
            ...
            webbrowser.open(url)
        # MEME
        elif bool_syn and ("meme" in query) and ("tell" or "show"):
            speak(Reply.okay())
            show_meme()
            bool_syn = False
            speak("Here is a Meme. Enjoy it")
        # ABOUT ME
        elif bool_syn and exist(['your boss', 'your owner', 'your developer', 'your god', 'your master']):
            bool_syn = False
            speak(f"You, are the only one In this entire world who can talk with. MISS {assi.name}")
        elif bool_syn and exist(['tell me about me', 'tell me about myself', 'do you know me', 'who am i']):
            speak(Reply.okay())
            ...
            bool_syn = False
        # ABOUT YOURSELF
        elif bool_syn and ('what is your name' in query) or ('you have any name' in query):
            bool_syn = False
            speak(f"My name {assi.name}")
        elif bool_syn and exist(['tell me about you', 'tell me about yourself', 'do you know you', 'who are you']):
        ...
        # TELL ABOUT.....
            ...
            if exist(['alexa', 'siri', 'google', 'Google']):
                ...
                wiki(query)
        # YOU ARE
        elif bool_syn and exist(['you are', 'your']) and exist(
                ['awesome', 'crazy', 'wonderful', 'amazing', 'insane', 'good', 'nice',
                 'kind', 'cool', 'better']):
            speak(Reply.thank())
            bool_syn = False
        # ENTERTAIN
        elif bool_syn and exist(['draw', 'entertain me', 'star']):
            bool_syn = False
        ...
        print("Query : ", query)
        # OPEN THINGS --------------------------------------------------------------------------------------------------
        if bool_open and exist(['open', 'run']):
            bool_open = False
        ...
        if bool_close and exist(['close', 'kill']):
            ...
            if ('yes' in confirm) or ('sure' in confirm):
                ...
                # VISUAL STUDIO
                    ...
                    speak(Reply.okay())
                    speak(f"{user.name}. I am Closing Visual Studio")
                    try:
                ...
                # PYCHARM
                    ...
                    speak(Reply.okay())
                    speak(f"{user.name}. I am Closing pycharm")
                    try:
                ...
                # NOTEPAD
                    ...
                    speak(Reply.okay())
                    speak(f"{user.name}. I am Closing notepad only for you")
                    try:
                ...
                # CALCUALTOR
                    ...
                    speak(Reply.okay())
                    speak(f"{user.name}. I am Closing calculator only for you")
                    try:
                ...
                # WORDPAD
                    ...
                    speak(Reply.okay())
                    speak(f"{user.name}. I am Closing WordPad only for you")
                    try:
                ...
                # BROWSER
                    ...
                    speak(Reply.okay())
                    speak(f"{user.name}. I am Closing Microsoft edge.")
                    try:
                ...
                # WEBSITES -----------------------------------------------------------------------------------------------------
                    ...
                    speak(Reply.okay())
                    speak("Should I close the Current Tab")
                    key = take_command().lower
                    if key == 'yes':
        ...
        if bool_read_write and exist(['read text', 'text to speech', 'read for me', 'open text reader', 'text reader']):
            ...
            # NOTE WRITER
        # NOTE MAKER
        elif bool_read_write and exist(
                ['write it down', 'make a note', 'write note', 'write it', 'right it', 'note it']):
            try:
                note = ""
        ...
        if bool_shot and exist(['take', 'capture']) and exist(['screenshot', 'snapshot', 'screen']):
            try:
                ...
                bool_shot = False
        # EXCITING
        if bool_good and exist(
                ['awesome', 'wonderful', 'amazing', 'exciting', 'wow', 'cool', 'nice', 'good', 'kind', 'crazy']):
            speak("Yaah, It is " + query)
            bool_good = False
        # BLESSED
        elif bool_thank and exist(['thanks', 'thank you']):
            speak(Reply.pleasure())
            bool_thank = False
        # NO
        elif bool_no and exist(['no', 'nope']):
            speak("okay")
            bool_no = False
        # BYE
        if bool_by and exist(['by', 'bye', 'then by', 'okay by']):
            by()
            bool_by = False
        elif bool_by and exist(['wait']):
            speak("Okay Grace is going to sleep. Tata")
            time.sleep(30)
        elif bool_by and exist(['pause', 'stop']):
            keyboard.press('space bar')
        # OKAY, GOT IT ETC....................
        elif bool_good and exist(
                ['ok', 'sure', 'definitely', 'yup', "yep", "respectable", "of course", "green light", "confirm",
                 "okeydokey", "surely", "satisfactory", "tolerable", "correct", "good", "not bad", "up to scratch",
                 "accurate", "no problem", "alright", "yes", "Here we go", "sounds good", "alright then", "absolutely",
                 "as you say"]):
            speak(Reply.okay())
            bool_good = False
        if bool_im and exist(['I am', "I'm"]):
            bool_im = False
        ...
        elif bool_wow and exist(['know', 'am awesome', 'am amazing', 'am great']):
            lst = [f'Yes you are awesome {user.name}', 'We are amazing', 'No one can beat us', 'You are Unstoppable']
            word = lst[random.randint(0, len(lst) - 1)]
            speak(word)
            bool_wow = False
        # WEATHER
        if bool_weather and exist(['weather', 'mosam', 'outside', 'about city', 'weather details']):
            weather.weather("indore")
            bool_weather = False
        # MATHS
        if bool_math and exist(
                ['+', '-', '*', 'x', '/', 'power', 'plus', 'minus', 'into', 'multiply', 'divide', 'calculate']):
            speak(Reply.okay())
        ...
        # OTHER --------------------------------------------------------------------------------------------------------
        # CHANGE NAME
        if bool_im and exist(['my name is']):
            user.set_name(query)
            speak(Reply.okay())
            speak(f"So, Your name is, {user.name}")
            bool_im = False
        # BLUETOOTH, CAMERA, WIFI, LIGHTS
        elif bool1 and exist(['can you', 'turn on', 'turn off']):
            bool1 = False
        ...
        elif bool3 and exist(['select all']):
            bool3 = False
            keyboard.press_and_release('ctrl + a')
            speak("Selected")
        if bool4 and exist(['cut']):
            bool4 = False
            keyboard.press_and_release('ctrl + x')
            speak("Cutted")
        if bool5 and exist(['copy']):
            bool5 = False
            keyboard.press_and_release('ctrl + c')
            speak("Copied")
        if bool6 and exist(['paste']):
            bool6 = False
            keyboard.press_and_release('ctrl + v')
            speak("Pasted")
        if bool6 and exist(['delete']):
            bool6 = False
            keyboard.press_and_release('backspace')
            speak("Deleted")
        if bool6 and exist(['undo']):
            bool6 = False
            keyboard.press_and_release('ctrl + z')
            speak("Done Boss")
        if (bool_weather and bool_wiki and bool_good and bool_hello and bool_songs and bool_open) and \
                ...
                (
                        bool_featuers and bool_no and bool_im and bool_good and bool_wow and bool_no and bool_thank and bool_math) and \
                (bool1 and bool2 and bool3 and bool4 and bool5 and bool6 and bool7):
